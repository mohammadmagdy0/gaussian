...
function setLanguage(lang) {
    ...
    for (let i = 0; i < currentRows; i++) {
        ...
        for (let j = 0; j <= currentCols; j++) {
            ...
            const input =
                document.getElementById(`m-${i}-${j}`);

            if (input) {

                input.placeholder =
                    j === currentCols
                    ? translations[lang].placeholder_b
                    : `x${j+1}`;
            }
        }
    }
...
function generateMatrix() {
    ...
    if (currentRows <= 0) {
        ...
        return alert(
            translations[currentLang].alert_dim
        );
    }
    ...
    document.getElementById('matrix-section')
    ...
    for (let i = 0; i < currentRows; i++) {
        ...
        for (let j = 0; j <= currentCols; j++) {
            ...
            input.placeholder =
                j === currentCols
                ? translations[currentLang].placeholder_b
                : `x${j+1}`;
...
function solve() {
    ...
    for (let i = 0; i < currentRows; i++) {
        ...
        for (let j = 0; j <= currentCols; j++) {
            ...
            if (
                ...
                alert(
                    translations[currentLang].alert_fill
                );
    ...
    for (let i = 0; i < currentRows; i++) {
        ...
        for (let j = 0; j < currentCols; j++) {
            ...
            if (
                Math.abs(workingMatrix[i][j]) > 0.000001
            ) {
...
function renderStep(matrix, title, notation) {
    ...
    let dynamicExplanation =
        currentLang === 'ar'
        ?
        `للتخلص من العنصر في <strong>الصف ${notation.split('=')[0].trim()}</strong>`
        :
        `To eliminate the element in <strong>${notation.split('=')[0].trim()}</strong>`;

    let matrixHTML =
        '<div class="matrix-wrapper" style="padding:0;">';
    ...
    matrix.forEach((row, i) => {
        ...
        row.forEach((val, j) => {
            ...
            if (j === currentCols) {
                ...
                matrixHTML += '<div class="v-line"></div>';
            }
    ...
    stepCard.innerHTML = `
        <div class="step-header">
            ...
            <span class="notation-badge" dir="ltr">
                ${notation}
            </span>
...
function toggleChat() {
    ...
    .classList.toggle('ai-chat-closed');
}
...
function exportToPDF() {
    ...
    if (
        ...
        return alert(
            translations[currentLang].alert_pdf
        );
    }
...
function copyMatrix(event) {
    ...
    navigator.clipboard.writeText(matrixData)
    .then(() => {
